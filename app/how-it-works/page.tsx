...
function FlowBlock({ index, title, items }: { index: string; title: string; items: string[] }) {
  return (
    <div className="grid md:grid-cols-4 gap-8 py-10 border-t" style={{ borderColor: C.surfaceLine }}>
      <div className="md:col-span-1">
        <span className="px-3 py-1 text-xs font-bold inline-block" style={{ background: C.primaryTint, color: C.primaryDark, fontFamily: F_DISPLAY, ...rounded(999) }}>
          {index}
        </span>
        <h3 className="mt-3 text-2xl" style={{ fontFamily: F_DISPLAY, fontWeight: 800, color: C.ink }}>{title}</h3>
      </div>
          ...
          {items.map((it, i) => (
            <li key={i} className="flex gap-4">
              <span
                ...
                style={{ background: C.surface, color: C.primary, fontFamily: F_DISPLAY, ...rounded(999) }}
              >
                {i + 1}
              </span>
  ...
  return (
    <div>
      <PageHeader
        ...
        title="Three flows, one continuous draft."
        sub="Onboarding feeds the editor, the editor feeds the cover letter — you never start from a blank page twice."
      />
      <div className="max-w-6xl mx-auto px-6 pb-16">
        <FlowBlock index="A" title="Onboarding → First CV" items={[
          ...
          "Answer the AI intake questionnaire — target job title, industry, experience level.",
        ...
        <FlowBlock index="B" title="CV Editor" items={[
          ...
          "Sections: Contact Info → Summary → Work Experience → Education → Skills → custom sections.",
          ...
          "The ATS checker panel scores the draft in real time.",
        ...
        <FlowBlock index="C" title="Cover Letter" items={[
          ...
          "The AI drafts a full letter — you edit inline and pick a tone.",
