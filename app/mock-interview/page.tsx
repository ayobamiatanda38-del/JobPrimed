  ...
  return (
    <div className={`flex ${isAI ? "justify-start" : "justify-end"} mb-3`}>
      <div
        ...
        style={{ background: isAI ? C.surface : C.primary, color: isAI ? C.ink : C.paper, fontFamily: F_BODY, ...rounded(16) }}
      >
        {children}
      </div>
    </div>
  ...
  return (
    <div className="mb-4">
      <div className="flex justify-between mb-1">
        ...
        <span style={{ fontFamily: F_BODY, fontSize: 12, color: C.graphite }}>{value}/100</span>
      </div>
      <div style={{ height: 8, background: C.surface, ...rounded(999) }}>
        <div style={{ height: 8, width: `${value}%`, background: C.primary, ...rounded(999) }} />
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div className="max-w-6xl mx-auto px-6 mb-8">
        <Link
          ...
          style={{ background: C.gold, ...rounded(16) }}
        >
          <span className="flex items-center gap-2" style={{ fontFamily: F_DISPLAY, fontWeight: 700, color: C.paper, fontSize: 14 }}>
            <Star size={16} fill={C.paper} /> Want a real 1-on-1? Try the live AI avatar interview — Premium
          </span>
          <span style={{ fontFamily: F_DISPLAY, fontWeight: 700, color: C.paper, fontSize: 13 }}>See it →</span>
        </Link>
      </div>
        ...
        <div>
          <div className="flex gap-2 mb-4">
            {["Behavioral", "Technical", "Screening"].map((t) => (
              <button key={t} onClick={() => setType(t)} className="px-4 py-2 text-xs font-semibold"
                style={{ fontFamily: F_DISPLAY, background: type === t ? C.ink : C.surface, color: type === t ? C.paper : C.graphite, ...rounded(999) }}>
                {t}
              </button>
            ))}
          </div>
          <Card className="p-5">
            <div style={{ fontFamily: F_BODY, fontSize: 11, color: C.graphiteLight, fontWeight: 600 }} className="mb-3">
            ...
            <div className="mt-4 pt-4 border-t flex items-center gap-2" style={{ borderColor: C.surfaceLine }}>
              <input
                value={input}
                ...
                style={{ background: C.surface, color: C.ink, fontFamily: F_BODY, ...rounded(999) }}
              />
              <button onClick={sendMessage} className="w-9 h-9 flex items-center justify-center shrink-0" style={{ background: C.primary, ...rounded(999) }} aria-label="Send answer">
                <ArrowRight size={16} color={C.paper} />
              </button>
            </div>
          </Card>
          <div className="flex items-center justify-between mt-3">
            <p style={{ fontFamily: F_BODY, fontSize: 11, color: C.graphiteLight }}>Scripted mock replies for preview — not a live AI call.</p>
            <button onClick={retryQuestion} style={{ fontFamily: F_DISPLAY, fontWeight: 700, color: C.primary, fontSize: 12 }}>Reset question</button>
          </div>
        </div>
        <div>
          <Card className="p-7">
            ...
            <div className="mt-6 p-4" style={{ background: C.primaryTint, ...rounded(14) }}>
              <div style={{ fontFamily: F_DISPLAY, fontWeight: 700, color: C.primaryDark, fontSize: 13 }} className="mb-1">Improvement area</div>
